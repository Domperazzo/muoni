...
int main (int argc, char ** argv){
    ...
    // TGraph2D g2d;
    // g2d.SetTitle("Distribuzione iniziale muoni");
    ...
    //TH1F h_theta("h_theta", "h_theta", sqrt(N), -2*M_PI, 2*M_PI);
    //TH1F h_phi("h_phi", "h_phi", sqrt(N), -2*M_PI, 2*M_PI);

    int j=0;
    int conteggi = 0;
    while (j < N){
      muone muon(rand_theta(f_cos2, -M_PI/2, 0, 4/M_PI), rand_phi(0, 2*M_PI));
      // h_theta.Fill(muon.get_theta());
      // h_phi.Fill(muon.get_phi());
